...
class SaleOrderExt(models.Model):
    ...
    sale_order_category_approver_lines = fields.One2many(
        "sale.order.category.approvers", "order_id", readonly=True)


    requires_approval = fields.Boolean(compute="_compute_requires_approval")
    requires_category_approval = fields.Boolean(compute="_compute_requires_category_approval")

    current_user_approver = fields.Boolean(
        compute="_compute_current_user_approver")
    current_user_category_approver = fields.Boolean(
        compute="_compute_current_user_category_approver")
    ...
    def _compute_requires_category_approval(self):
        for rcd in self:
            if rcd.sale_order_category_approver_lines and rcd.category_approvals_is_active:
                if rcd.sale_order_category_approver_lines.filtered(lambda x: x.app_status == False):
                    rcd.requires_category_approval = True
                else:
                    rcd.requires_category_approval = False
            else:
                rcd.requires_category_approval = False
    ...
    def _compute_current_user_category_approver(self):
        for rcd in self:
            partner_object = rcd.sale_order_category_approver_lines.filtered(
                lambda x: x.name == self.env.user.partner_id and x.app_status == False)
            if partner_object:
                rcd.current_user_category_approver = True
            else:
                rcd.current_user_category_approver = False
    ...
    def category_approve_order(self):
        partner_approver_line_object = self.sale_order_category_approver_lines.filtered(
            lambda x: x.name == self.env.user.partner_id)
        partner_approver_line_object.app_status = True
        self.message_post(
            message_type='notification',
            subtype_id = self.env.ref('mail.mt_comment').id,
            body=f'{self.env.user.partner_id.name} has approved the category of this Quotation',
            partner_ids = [self.create_uid.partner_id.id],
            )
        ...
        for record in self:
            for approver_line in record.sale_order_approver_lines:
                ...
                self.message_post(
                    message_type='notification',
                    subtype_id = self.env.ref('mail.mt_comment').id,
                    body=f'Manager {self.env.user.partner_id.name} has approved this Quotation on behalf of: {approvers_string.join(record.sale_order_approver_lines.mapped("name.name"))}',
                    )

    def manager_category_approve_order(self):
        for record in self:
            for approver_line in record.sale_order_category_approver_lines:
                approvers_string = ', '
                approver_line.app_status = True
                self.message_post(
                    message_type='notification',
                    subtype_id = self.env.ref('mail.mt_comment').id,
                    body=f'Manager {self.env.user.partner_id.name} has approved the categories of this Quotation on behalf of: {approvers_string.join(record.sale_order_category_approver_lines.mapped("name.name"))}',
                    )
    ...
    @api.constrains("team_id", "order_line")
    def fill_sale_order_category_approver_lines(self):
        if self.category_approvals_is_active:
            order_categories = list(set([sol.product_id.categ_id.id for sol in self.order_line]))
            self.write({"sale_order_category_approver_lines": [(5, 0, 0)]})
            approvers = self.team_id.category_approvers_lines.filtered(lambda x: len(list(set(x.category_ids.ids) & set(order_categories))) > 0)
            if approvers:
                for approver in approvers:
                    self.write({
                        "sale_order_category_approver_lines": 
                            [(0, 0, {"name": approver.name.id})]
                        })
                    send_notification = self.env['ir.config_parameter'].sudo(
                    ...
                    if send_notification:
                        ...
                        body = f"""
                            ...
                            <div>
                                You have been assigned to approve the category of S.O.: {self.name}.
                            </div>
                            {link}
                        """
                        self.message_post(
                            body=body,
                            subject="Sale Order Category Approval",
                            subtype_id = self.env.ref('mail.mt_comment').id,
                            message_type='notification',
                            notification_ids=notification_ids
                            )
